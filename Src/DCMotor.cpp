...
void DCMotor::control_ramp_speed(void) {
    ...
    for(int i = 0; i < NB_MOTORS; i++){
        if(speed_order_limited[i] - speed[i] >= max_speed_delta) {
        	speed_command[i] = speed[i]+max_speed_delta;
        	debug[15] = 0;
        }
        else if (speed_order_limited[i] - speed[i] <= -max_speed_delta ) {
        	speed_command[i] = speed[i]-max_speed_delta;
        	debug[15] = 1;
        }
        else {
...
void DCMotor::limitLinearSpeedCmdByGoal()
{
	...
	compute_limit_in = UPDATE_RATE/UPDATE_RATE_limite_goal;
    ...
    else if (l_distance_to_goal > distance_to_stop + extra_distance)
    {
        ...
        new_speed_order = l_linear_speed + max_acceleration / float(UPDATE_RATE_limite_goal);
    }
    else
    {
    	debug[0] = 0.3f;
    ...
    debug[1] = extra_distance;
    debug[2] = l_linear_speed;
    debug[3] = new_speed_order;
    debug[4] = UPDATE_RATE_limite_goal;
    debug[5] = l_distance_to_goal;
    debug[6] = time_to_stop;
    debug[7] = distance_to_stop;
    ...
    debug[8] = static_cast<float>(speed_order_limited[M_L]);
    debug[9] = static_cast<float>(speed_order_limited[M_R]);
    debug[10] = static_cast<float>(new_speed_order_tick);
    debug[11] = static_cast<float>(speed_order[M_L]);
    debug[12] = static_cast<float>(speed_order[M_R]);
}
